...
public final class ContentCache 
{
	 ...
	 */
	private static ContentCache instance = new ContentCache();

	/**
	 * Maximum number of ContentBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of ContentBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual ContentBean.
	 */
	...
	*/
	...
	/**
	 * A ContentBean object is passed to this method for addition to the cache.
	 */
	public void addContent(ContentBean bean) 
	{
		synchronized(updateLock)
		{ 
			Integer id = bean.getContentId();
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getContentId());
						lru.next = null;
					} 
					else 
					{
	...
	/**
	 * Return the ContentBean associated with the given key.
	 */
	public ContentBean fetchContent(Integer id) 
	{
		...
		if (null == foundLink) 
		{
			...
			try 
			{
				bean = ContentManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates ContentBean object in the cache.
	 */
	public void updateContent(ContentBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isContentInCache(bean))  
			{
				Integer id = bean.getContentId();			
	...
	/**
	 * Returns true if ContentBean object is in the cache. FALSE otherwise
	 */
	public boolean isContentInCache(ContentBean bean) 
	{
		Integer id = bean.getContentId();
