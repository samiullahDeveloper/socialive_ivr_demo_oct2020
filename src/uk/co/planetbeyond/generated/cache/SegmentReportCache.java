 ...
 * @author afagot
 */
public final class SegmentReportCache 
{
	/**
	 * Unique instance of the cache manager.
	 */
	private static SegmentReportCache instance = new SegmentReportCache();

	/**
	 * Maximum number of SegmentReportBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of SegmentReportBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual SegmentReportBean.
	 */
	...
	*/
	 ...
	 */
	public static SegmentReportCache getInstance() 
	{
		return instance;
	}

	/**
	 * A SegmentReportBean object is passed to this method for addition to the cache.
	 */
	public void addSegmentReport(SegmentReportBean bean) 
	{
		synchronized(updateLock)
		{ 
			Integer id = bean.getSegmentReportId();
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getSegmentReportId());
						lru.next = null;
					} 
					else 
					{
			...
			{ 
				...
				fetchSegmentReport(id);
			}
		}
	}

	/**
	 * Return the SegmentReportBean associated with the given key.
	 */
	public SegmentReportBean fetchSegmentReport(Integer id) 
	{
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) 
		{
			SegmentReportBean bean = null;
			try 
			{
				bean = SegmentReportManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates SegmentReportBean object in the cache.
	 */
	public void updateSegmentReport(SegmentReportBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isSegmentReportInCache(bean))  
			{
				Integer id = bean.getSegmentReportId();			
	...
	/**
	 * Returns true if SegmentReportBean object is in the cache. FALSE otherwise
	 */
	public boolean isSegmentReportInCache(SegmentReportBean bean) 
	{
		Integer id = bean.getSegmentReportId();
