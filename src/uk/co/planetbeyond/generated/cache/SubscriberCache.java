...
public final class SubscriberCache 
{
	...
	/**
	 * Maximum number of SubscriberBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of SubscriberBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual SubscriberBean.
	 */
	...
	*/
	...
	/**
	 * A SubscriberBean object is passed to this method for addition to the cache.
	 */
	public void addSubscriber(SubscriberBean bean) 
	{
		synchronized(updateLock)
		{ 
			Integer id = bean.getSubscriberId();
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getSubscriberId());
						lru.next = null;
					} 
					else 
					{
	...
	/**
	 * Return the SubscriberBean associated with the given key.
	 */
	public SubscriberBean fetchSubscriber(Integer id) 
	{
		...
		if (null == foundLink) 
		{
			...
			try 
			{
				bean = SubscriberManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates SubscriberBean object in the cache.
	 */
	public void updateSubscriber(SubscriberBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isSubscriberInCache(bean))  
			{
				Integer id = bean.getSubscriberId();			
	...
	/**
	 * Returns true if SubscriberBean object is in the cache. FALSE otherwise
	 */
	public boolean isSubscriberInCache(SubscriberBean bean) 
	{
		Integer id = bean.getSubscriberId();
