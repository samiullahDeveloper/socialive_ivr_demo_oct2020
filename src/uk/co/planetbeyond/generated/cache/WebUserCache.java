 ...
 * @author afagot
 */
public final class WebUserCache 
{
	/**
	 * Unique instance of the cache manager.
	 */
	private static WebUserCache instance = new WebUserCache();

	/**
	 * Maximum number of WebUserBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of WebUserBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual WebUserBean.
	 */
	private HashMap<String, LinkedList> cache = new HashMap<String, LinkedList>();
	...
	*/
	 ...
	 */
	public static WebUserCache getInstance() 
	{
		return instance;
	}

	/**
	 * A WebUserBean object is passed to this method for addition to the cache.
	 */
	public void addWebUser(WebUserBean bean) 
	{
			...
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getUsername());
						lru.next = null;
					} 
					else 
					{
	...
	/**
	 * Return the WebUserBean associated with the given key.
	 */
	public WebUserBean fetchWebUser(String id) 
	{
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) 
		{
			WebUserBean bean = null;
			try 
			{
				bean = WebUserManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates WebUserBean object in the cache.
	 */
	public void updateWebUser(WebUserBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isWebUserInCache(bean))  
			{
				String id = bean.getUsername();			
	...
	/**
	 * Returns true if WebUserBean object is in the cache. FALSE otherwise
	 */
	public boolean isWebUserInCache(WebUserBean bean) 
	{
		String id = bean.getUsername();
