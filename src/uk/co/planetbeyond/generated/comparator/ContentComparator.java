




// ______________________________________________________
// Generated by sql2java - http://sql2java.sourceforge.net/
// jdbc driver used at code generation time: com.mysql.jdbc.Driver
//
// Author: Javed Kansi
// ______________________________________________________

package uk.co.planetbeyond.generated.comparator;

import java.util.Comparator;
import uk.co.planetbeyond.generated.ContentBean;
import uk.co.planetbeyond.generated.ContentManager;


/**
 * Comparator class is used to sort the ContentBean objects.
 * @author sql2java
 */
public class ContentComparator implements Comparator
{
    /**
     * Holds the field on which the comparison is performed.
     */
    private int iType;
    /**
     * Value that will contain the information about the order of the sort: normal or reversal.
     */
    private boolean bReverse;

    /**
     * Constructor class for ContentComparator.
     * <br>
     * Example:
     * <br>
     * <code>Arrays.sort(pArray, new ContentComparator(ContentManager.ID_MENU_NUMBER, bReverse));<code>
     *
     * @param iType the field from which you want to sort
     * <br>
     * Possible values are:
     * <ul>
     *   <li>ContentManager.ID_MENU_NUMBER
     *   <li>ContentManager.ID_SYNCHRONIZATION_TIMESTAMP
     *   <li>ContentManager.ID_STATUS
     *   <li>ContentManager.ID_FILENAME
     *   <li>ContentManager.ID_CREATION_TIMESTAMP
     *   <li>ContentManager.ID_CATEGORY_ID
     *   <li>ContentManager.ID_CONTENT_ID
     * </ul>
     */
    public ContentComparator(int iType)
    {
        this(iType, false);
    }

    /**
     * Constructor class for ContentComparator.
     * <br>
     * Example:
     * <br>
     * <code>Arrays.sort(pArray, new ContentComparator(ContentManager.ID_MENU_NUMBER, bReverse));<code>
     *
     * @param iType the field from which you want to sort.
     * <br>
     * Possible values are:
     * <ul>
     *   <li>ContentManager.ID_MENU_NUMBER
     *   <li>ContentManager.ID_SYNCHRONIZATION_TIMESTAMP
     *   <li>ContentManager.ID_STATUS
     *   <li>ContentManager.ID_FILENAME
     *   <li>ContentManager.ID_CREATION_TIMESTAMP
     *   <li>ContentManager.ID_CATEGORY_ID
     *   <li>ContentManager.ID_CONTENT_ID
     * </ul>
     *
     * @param bReverse set this value to true, if you want to reverse the sorting results
     */
    public ContentComparator(int iType, boolean bReverse)
    {
        this.iType = iType;
        this.bReverse = bReverse;
    }

    /**
     * Implementation of the compare method.
     */
    public int compare(Object pObj1, Object pObj2)
    {
        ContentBean b1 = (ContentBean)pObj1;
        ContentBean b2 = (ContentBean)pObj2;
        int iReturn = 0;
        switch(iType)
        {
            case ContentManager.ID_MENU_NUMBER:
                if (b1.getMenuNumber() == null && b2.getMenuNumber() != null) {
                    iReturn = -1;
                } else if (b1.getMenuNumber() == null && b2.getMenuNumber() == null) {
                    iReturn = 0;
                } else if (b1.getMenuNumber() != null && b2.getMenuNumber() == null) {
                    iReturn = 1;
                } else {
                    iReturn = b1.getMenuNumber().compareTo(b2.getMenuNumber());
                }
                break;
            case ContentManager.ID_SYNCHRONIZATION_TIMESTAMP:
                if (b1.getSynchronizationTimestamp() == null && b2.getSynchronizationTimestamp() != null) {
                    iReturn = -1;
                } else if (b1.getSynchronizationTimestamp() == null && b2.getSynchronizationTimestamp() == null) {
                    iReturn = 0;
                } else if (b1.getSynchronizationTimestamp() != null && b2.getSynchronizationTimestamp() == null) {
                    iReturn = 1;
                } else {
                    iReturn = b1.getSynchronizationTimestamp().compareTo(b2.getSynchronizationTimestamp());
                }
                break;
            case ContentManager.ID_STATUS:
                if (b1.getStatus() == null && b2.getStatus() != null) {
                    iReturn = -1;
                } else if (b1.getStatus() == null && b2.getStatus() == null) {
                    iReturn = 0;
                } else if (b1.getStatus() != null && b2.getStatus() == null) {
                    iReturn = 1;
                } else {
                    iReturn = b1.getStatus().compareTo(b2.getStatus());
                }
                break;
            case ContentManager.ID_FILENAME:
                if (b1.getFilename() == null && b2.getFilename() != null) {
                    iReturn = -1;
                } else if (b1.getFilename() == null && b2.getFilename() == null) {
                    iReturn = 0;
                } else if (b1.getFilename() != null && b2.getFilename() == null) {
                    iReturn = 1;
                } else {
                    iReturn = b1.getFilename().compareTo(b2.getFilename());
                }
                break;
            case ContentManager.ID_CREATION_TIMESTAMP:
                if (b1.getCreationTimestamp() == null && b2.getCreationTimestamp() != null) {
                    iReturn = -1;
                } else if (b1.getCreationTimestamp() == null && b2.getCreationTimestamp() == null) {
                    iReturn = 0;
                } else if (b1.getCreationTimestamp() != null && b2.getCreationTimestamp() == null) {
                    iReturn = 1;
                } else {
                    iReturn = b1.getCreationTimestamp().compareTo(b2.getCreationTimestamp());
                }
                break;
            case ContentManager.ID_CATEGORY_ID:
                if (b1.getCategoryId() == null && b2.getCategoryId() != null) {
                    iReturn = -1;
                } else if (b1.getCategoryId() == null && b2.getCategoryId() == null) {
                    iReturn = 0;
                } else if (b1.getCategoryId() != null && b2.getCategoryId() == null) {
                    iReturn = 1;
                } else {
                    iReturn = b1.getCategoryId().compareTo(b2.getCategoryId());
                }
                break;
            case ContentManager.ID_CONTENT_ID:
                if (b1.getContentId() == null && b2.getContentId() != null) {
                    iReturn = -1;
                } else if (b1.getContentId() == null && b2.getContentId() == null) {
                    iReturn = 0;
                } else if (b1.getContentId() != null && b2.getContentId() == null) {
                    iReturn = 1;
                } else {
                    iReturn = b1.getContentId().compareTo(b2.getContentId());
                }
                break;
            default:
                throw new IllegalArgumentException("Type passed for the field is not supported");
        }

        return bReverse ? (-1 * iReturn) : iReturn;
    }}
